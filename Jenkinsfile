   ...
   stages {
    ...
    stage('Run container') {
      steps {   
              script {
		      LAST_STARTED = env.STAGE_NAME
          	sh '/Applications/Docker.app/Contents/Resources/bin/docker run -p 8082:8082 --name apiops-anypoint-jenkins-sapi sivendu/apiops-anypoint-jenkins-sapi:latest'
	      }
      }
    }
    /*    stage ('Munit Test'){
